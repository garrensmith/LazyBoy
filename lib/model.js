...
var Model = function (model_type) {
  var model_thing = defined_models[model_type];
  ...
  if (model_thing.instance) {
    return model_thing.instance;
  }

  model_thing.instance = new ModelDocument(model_type, model_thing.schema); 
...
Model.define = function (model_type, model_schema) {
  ...
  var model_thing = {};
  model_thing.schema = schema;
  model_thing.instance = new ModelDocument(model_type, schema);

  defined_models[model_type] = model_thing;

  logger.info("Defined schema  %s for %s", schema, model_type); 

  // should store in instance
  Model.create_views(model_type, schema);
